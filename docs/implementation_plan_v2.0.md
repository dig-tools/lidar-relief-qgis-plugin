...
The current commit (`1a80682`) has all v1.3.5 code pushed to both
GitHub and GitLab. We'll work on a new `v2.0-dev` branch (or a linked
worktree) for the v2.0 implementation to keep `master` clean.

Would you like to proceed? If so, I suggest we start with Phase 1.1
(COG Export) since it's self-contained, low-risk, and delivers
