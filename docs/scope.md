...
- Openness (positive and negative) algorithm
- Multi-scale Topographic Position (MSTP) — the most powerful anomaly detection visualisation
- Visualisation blend mode helper: create multi-layer composites (SVF+SLRM overlay) with preset blend modes
- Colour ramp presets optimised for archaeological anomaly detection
- Automatic feature detection: highlight pixels likely to represent archaeological features (earthworks, ridge-and-furrow, enclosures) using thresholding
- LiDAR point cloud ground classification using CSF (Cloth Simulation Filter) algorithm
- Integration with QGIS Atlas for systematic tile-by-tile processing of large landscapes
