...
def generate_report(
    ...
    styles.add(ParagraphStyle(
        "CoverTitle", fontSize=24, leading=30, spaceAfter=12,
        alignment=TA_CENTER, textColor=colors.HexColor("#2c3e50")
    ))
    styles.add(ParagraphStyle(
        "CoverSubtitle", fontSize=14, leading=18, spaceAfter=6,
        alignment=TA_CENTER, textColor=colors.HexColor("#7f8c8d")
    ))
    styles.add(ParagraphStyle(
        "SectionHead", fontSize=16, leading=20, spaceBefore=16,
        spaceAfter=8, textColor=colors.HexColor("#2c3e50"),
        borderWidth=0, borderColor=colors.HexColor("#3498db"),
        borderPadding=4,
    ))
    styles.add(ParagraphStyle(
        "Field", fontSize=10, leading=14, spaceAfter=2,
        textColor=colors.HexColor("#555555"),
    ))
    styles.add(ParagraphStyle(
        "Mono", fontSize=8, leading=10, spaceAfter=2,
        fontName="Courier", leftIndent=8,
    ))
    styles.add(ParagraphStyle(
        "Footer", fontSize=8, leading=10, alignment=TA_CENTER,
    ...
    story.append(Paragraph(
        f"Algorithm: <b>{algorithm_name}</b>", styles["CoverSubtitle"]
    ))
    if author:
    ...
    if plugin_version:
        story.append(Paragraph(
            f"Plugin version: {plugin_version}", styles["CoverSubtitle"]
        ))
    ...
    story.append(Paragraph(
        ...
        ParagraphStyle(
            "Abstract", fontSize=10, leading=14,
    ...
    param_data.append(["Algorithm", algorithm_name])
    param_data.append(["Plugin Version", plugin_version or "—"])
    param_data.append(["Processing Date", datetime.now().strftime('%Y-%m-%d %H:%M:%S')])
    ...
    for key, value in sorted(algorithm_params.items()):
        ...
        param_data.append([display_key, display_val])
    ...
    meta_data.append(["Source Raster", os.path.basename(raster_path)])

    meta_fields = [
        ("CRS", metadata.get("crs", "—")),
        ("Resolution", metadata.get("resolution", "—")),
        ("Extent (xmin, ymin, xmax, ymax)",
         str(metadata.get("extent", "—"))),
    ...
    if include_stats or include_histogram:
        ...
        if stats:
            story.append(Spacer(1, 8 * mm))
            story.append(Paragraph(
                "3. Output Statistics", styles["SectionHead"]
            ))
            story.append(Spacer(1, 4 * mm))
    ...
    story.append(Paragraph(
        ...
        ParagraphStyle(
            "Disclaimer", fontSize=8, leading=10,
...
def _generate_histogram_image(
    ...
    for i, val in enumerate(hist):
        ...
        x = margin_l + i * bar_w
        y = margin_b
        d.add(Rect(x, y, max(bar_w * 0.8, 1), max(bar_h, 1),
                   fillColor=colors.HexColor("#3498db"),
                   strokeColor=None))

    # Axis labels
    d.add(Line(margin_l, margin_b, margin_l, margin_b + chart_h,
               strokeColor=colors.HexColor("#333333"), strokeWidth=1))
    d.add(Line(margin_l, margin_b, margin_l + chart_w, margin_b,
               strokeColor=colors.HexColor("#333333"), strokeWidth=1))

    # Min/max labels
    d.add(String(margin_l, margin_b - 12, f"{edges[0]:.2f}",
                 fontSize=8, fontName="Helvetica"))
    d.add(String(margin_l + chart_w - 30, margin_b - 12, f"{edges[-1]:.2f}",
                 fontSize=8, fontName="Helvetica"))
